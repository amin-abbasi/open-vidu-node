...
app.use(session({ saveUninitialized: true, resave: false, secret: 'MY_SECRET' }))
...
// Entry point to OpenVidu Node Client SDK
var OV = new OpenVidu(process.env.OPENVIDU_URL, process.env.OPENVIDU_SECRET)
  ...
  if (!isLogged(req.session)) {
    ...
    if (mapSessions[sessionName]) {
      ...
      mySession.generateToken(tokenOptions)
        .then(result => {
          const token = result
          // const token = _.replace(result, 'localhost', '192.168.30.103')
          console.log('>>>>>>>>> Existing session token ', token)
          // Store the new token in the collection of tokens
          mapSessionNamesTokens[sessionName].push(token)

          // Return the token to the client
          res.status(200).send({ 0: token })
        })
        .catch(error => {
          console.log(error)
        })
...
function getBasicAuth() {
  return 'Basic ' + (new Buffer('OPENVIDU APP:' + OPENVIDU_SECRET).toString('base64'))
}
